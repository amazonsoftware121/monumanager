...
const deleteCarving = (req, res) =>{
   const carvingId = req.params.id;
   const q = "DELETE FROM carving WHERE `id` = ?";

   db.query(q, [carvingId], (err, data) =>{
    if(err) return res.status(403).json("Can't deleted");
    return res.json("Carving has been deleted.");
   })
}
const editCarving = (req, res) => {
    const carvingId = req.params.carvingId; 
    const q = "UPDATE carving SET `side` = ?, `position` = ?, `first_name` = ?, `middle_name` = ?, `last_name` = ?, `birth_date` = ?, `passing_date` = ?, `other_details` = ? WHERE `id` = ?";
    ...
    db.query(q, [...values,carvingId], (err, data) => {
        if (err) return res.status(500).json(err);
        return res.status(200).json("Carving Details updated");
    });
}

module.exports = {addCarving,getCarvings,getCarving,deleteCarving,editCarving };