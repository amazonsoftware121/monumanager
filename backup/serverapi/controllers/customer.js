...
const updateCustomer = (req, res) => {
    ...
    const qsel = `SELECT * FROM customer WHERE customer.id = ?`;
    db.query(qsel, [customerId], (err, data) => {
      if (err) {
      ...
      let phone, first, midal, last, note, address;
      ...
      if (req.body.midal == null) {
        midal = data[0].midal;
      } else {
        midal = req.body.midal;
      }
      ...
      const q = `UPDATE customer SET phone=?, first=?, midal=?, last=?, note=?, address=? WHERE id=?`;
      db.query(q, [phone, first, midal, last, note, address, customerId], (err, result) => {
        if (err) {
          // Handle the error appropriately
          console.error(err);
          return res.status(500).send('An error occurred');
...
module.exports = {getCustomers,updateCustomer, addCustomer,deleteCustomer,getSingleCustomer};